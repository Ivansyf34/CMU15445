...
void InsertHelper(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> &tree, const std::vector<int64_t> &keys,
...
void InsertAndGetHelper(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> &tree, const std::vector<int64_t> &keys,
                        __attribute__((unused)) uint64_t thread_itr = 0) {
  GenericKey<16> index_key;
  RID rid;
  // create transaction
...
void InsertHelperSplit(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> &tree, const std::vector<int64_t> &keys,
...
void DeleteHelper(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> &tree, const std::vector<int64_t> &remove_keys,
                  __attribute__((unused)) uint64_t thread_itr = 0) {
  GenericKey<16> index_key;
  // create transaction
  Transaction *transaction = new Transaction(0);
...
void DeleteHelperSplit(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> &tree,
...
TEST(BPlusTreeConcurrentTest, InsertTest1) {
  ...
  auto key_scheam = ParseCreateStatement("a bigint");
  GenericComparator<16> comparator(key_scheam.get());
...
TEST(BPlusTreeConcurrentTest, MixTest) {
  ...
  // concurrent insert
  keys.clear();
  for (int i = 6; i <= 10; i++) keys.push_back(i);
