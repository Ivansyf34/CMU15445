...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    bool tag = false;
    while (c != key.end()) {
      if (tag || !node->get()->HasChild(*c)) {
        node = node->get()->InsertChildNode(*c, std::make_unique<TrieNode>(*c));
        tag = true;
      } else {
    ...
    auto new_node = new TrieNodeWithValue(std::move(**node), value);
    node->reset(new_node);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    if (node->get()->HasChildren()) {
      auto new_node = new TrieNode(std::move(**node));
      node->reset(new_node);
      latch_.WUnlock();
      return true;
    }
